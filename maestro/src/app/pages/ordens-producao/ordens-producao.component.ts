...
export class OrdensProducaoComponent implements OnInit {
  ...
  // Propriedades para impressão em massa
  imprimirIdsInput = '';
  ...
  handleKeyboardEvent(event: KeyboardEvent): void {
    ...
    if (event.key === 'Escape') {
      if (this.showReprogramPopup) {
        ...
        event.preventDefault();
      } else if (this.showImprimirModal) {
        this.closeImprimirModal();
        event.preventDefault();
    ...
    if (event.key === 'Enter' && !event.shiftKey && !event.ctrlKey && !event.altKey) {
      ...
      if (this.showReprogramModal && !this.isProcessing && this.parsedIdsCount > 0 && this.canReprogramWithDate()) {
        ...
        event.preventDefault();
      } else if (this.showImprimirModal && !this.isLoadingPrintFiles && this.parsedImprimirIdsCount > 0 && this.arquivosParaImprimir.length === 0) {
        this.buscarArquivosParaImprimir();
        event.preventDefault();
  ...
  openRoutine(routineName: string): void {
    ...
    if (routineName === 'reprogramar-massa') {
      ...
      this.openPrintModal();
    } else if (routineName === 'impressao-massa') {
      this.openImprimirModal();
  ...
  openImprimirModal(): void {
    this.showImprimirModal = true;
    this.imprimirIdsInput = '';
    this.parsedImprimirIdsCount = 0;
    this.arquivosParaImprimir = [];
    this.isLoadingPrintFiles = false;
  ...
  onImprimirIdsInput(): void {
    const ids = this.parseIds(this.imprimirIdsInput);
    this.parsedImprimirIdsCount = ids.length;
    this.cdr.detectChanges();
  }

  async buscarArquivosParaImprimir(): Promise<void> {
    const ids = this.parseIds(this.imprimirIdsInput);
    
    if (ids.length === 0) {
      this.resultType = 'error';
      this.resultMessage = 'Nenhum ID válido encontrado';
      return;
    }

    this.isLoadingPrintFiles = true;
    this.printProgress = 0;
    this.resultMessage = 'Buscando arquivos...';
    this.resultType = '';
    this.arquivosParaImprimir = [];
    this.cdr.detectChanges();

    try {
      const response = await this.jiraService.buscarArquivosPorIds(ids).toPromise();
      
      if (response && response.success) {
        const allFiles = response.files;
        
        // Filtrar apenas PDFs
        const pdfFiles = allFiles.filter((file: any) => {
        ...
        if (pdfFiles.length === 0) {
          this.resultType = 'error';
          this.resultMessage = 'Nenhum arquivo PDF encontrado para os IDs fornecidos';
          this.isLoadingPrintFiles = false;
          this.cdr.detectChanges();
          return;
        }
        ...
        this.resultMessage = `Baixando ${pdfFiles.length} PDF(s)...`;
        this.cdr.detectChanges();
        ...
        for (let i = 0; i < pdfFiles.length; i++) {
          ...
          try {
            ...
            if (blob) {
              this.arquivosParaImprimir.push({
                url: file.url,
                name: file.name,
                cardId: file.cardId,
                selected: true,
          ...
          this.printProgress = Math.round(((i + 1) / pdfFiles.length) * 100);
          this.resultMessage = `Baixando PDFs... ${this.printProgress}% (${i + 1}/${pdfFiles.length})`;
          this.cdr.detectChanges();
        }

        if (this.arquivosParaImprimir.length > 0) {
          this.resultType = 'success';
          this.resultMessage = `${this.arquivosParaImprimir.length} PDF(s) prontos para impressão. Selecione os arquivos desejados.`;
        } else {
          this.resultType = 'error';
          this.resultMessage = 'Erro ao baixar os arquivos PDF';
        }
        this.cdr.detectChanges();
      } else {
        this.resultType = 'error';
        this.resultMessage = 'Nenhum arquivo encontrado';
        this.cdr.detectChanges();
      }
    } catch (error) {
      console.error('Erro ao buscar arquivos:', error);
      this.resultType = 'error';
      this.resultMessage = 'Erro ao buscar arquivos: ' + (error as any).message;
      this.cdr.detectChanges();
    } finally {
      this.isLoadingPrintFiles = false;
      this.printProgress = 0;
  ...
  toggleSelectArquivoImprimir(index: number): void {
    this.arquivosParaImprimir[index].selected = !this.arquivosParaImprimir[index].selected;
    this.cdr.detectChanges();
  }

  toggleSelectAllImpressao(selectAll: boolean): void {
    this.arquivosParaImprimir.forEach(file => {
      file.selected = selectAll;
    });
    this.cdr.detectChanges();
  }

  getSelectedImpressaoCount(): number {
  ...
  async imprimirManual(): Promise<void> {
    ...
    this.resultMessage = `Preparando ${selectedFiles.length} arquivo(s) para impressão...`;
    this.resultType = '';
    this.cdr.detectChanges();
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        if (file.blob) {
          this.resultMessage = `Imprimindo arquivo ${i + 1} de ${selectedFiles.length}: ${file.name}`;
          this.cdr.detectChanges();
          
          await this.printPdfSilently(file.blob, file.name);
          
          // Aguardar um pouco entre cada arquivo
...
- Marcar "Lembrar minhas escolhas" para não aparecer novamente`;
      ...
      this.resultType = 'error';
      this.resultMessage = 'Erro ao preparar arquivos para impressão: ' + (error as any).message;
      this.cdr.detectChanges();
    } finally {
      this.isPrinting = false;
      this.cdr.detectChanges();
